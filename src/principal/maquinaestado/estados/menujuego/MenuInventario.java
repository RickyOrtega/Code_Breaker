...
public class MenuInventario extends SeccionMenu {
    ...
    private void dibujarRejillaInventario(final Graphics g, EstructuraMenu em) {
        ...
        for (int i = 0; i < 12; i++) {
            for (int j = 0; j < 7; j++) {
                DibujoDebug_R.dibujarRectangulo(g, puntoInicial.x + i * (Constantes.LADO_SPRITE + MARGEN_GENERAL) , puntoInicial.y + j * (Constantes.LADO_SPRITE + MARGEN_GENERAL), Constantes.LADO_SPRITE, Constantes.LADO_SPRITE, Color.BLACK, true);
            }
        }
    }
    ...
    private void dibujarElementosInventario(final Graphics g, EstructuraMenu em) {
        ...
        for (int i = 0; i < inventario.getObjetos().size(); i++) {
            DibujoDebug_R.dibujarImagen(g, inventario.getObjetos().get(i).getSprite().getImage(), puntoInicial.x + i * (Constantes.LADO_SPRITE + MARGEN_GENERAL) , puntoInicial.y);
            g.setColor(Color.BLACK);
            DibujoDebug_R.dibujarRectangulo(g, puntoInicial.x + i * (Constantes.LADO_SPRITE + MARGEN_GENERAL) +Constantes.LADO_SPRITE -12,
            ...
            DibujoDebug_R.dibujarString(g, texto, puntoInicial.x + i * (Constantes.LADO_SPRITE + MARGEN_GENERAL) + Constantes.LADO_SPRITE - (MedidorString.medirAnchoPixeles(g, texto)*4) - Constantes.LADO_SPRITE/16,
    ...
    private void dibujarPaginador(final Graphics g, EstructuraMenu em){
        ...
        DibujoDebug_R.dibujarRectangulo(g, anterior, Constantes.ROJO_GLOBAL, true);
        DibujoDebug_R.dibujarRectangulo(g, siguiente, Constantes.ROJO_GLOBAL, true);
    }
